...
1. Все диагностики стадий 1–2 — **только ошибки, предупреждений не существует** (проверено по
   всем местам `new Errore(` — 450 в `Int/`, из них ~16 в закомментированном блоке
   `StructErrorParser`; сводка в §5; grep `warning` по `Int/` — пусто).
...
6. Из **182** зарегистрированных кодов `ErrorsCodeList` **37 кодов — мёртвые** (никогда не
   возбуждаются живым кодом), §6. Ещё 3 (1807, 1820, 1821) возбуждаются только из
   закомментированного кода; живых кодов — 142. Для порта их можно не реализовывать, но словарь текстов стоит
   сохранить целиком (нумерация должна совпадать с C#-словарём).
7. Код 1912 есть только в RU-словаре: **в UA и EN он пропущен**
   (`DT/ErrorsCodeList.cs:377-380` UA и `:581-584` EN — после 1911 сразу 1913):
   `GetError(1912)` вернёт пустую строку (§3.2, §5.8).
...
Для кода 1912 в UA и EN `GetError` вернёт `""` → текст будет `... | code: 1912 ===>  ( `+
...
| 1912 ● | два операнда подряд без оператора — `P/LogicErrorParser.cs:45-49,:123-127,:205-209,:354-358`; `and/or/then` сразу после `and/or` — `:438-446` | В логическом выражении должно быть два операнда | — (в SetEN код 1912 не зарегистрирован) | — (в SetUA код 1912 не зарегистрирован) |
...
**Примечание к 1912.** Код 1912 зарегистрирован только в SetRU (`ErrorsCodeList.cs:174`);
SetEN перескакивает с 1911 на 1913 (`ErrorsCodeList.cs:581-584`), SetUA — тоже
(`:377-380`). Для 1912 UA/EN дадут пустую фразу после `===>`
(`GetError` возвращает `""`, `ErrorsCodeList.cs:624-634`).
...
- Каталог: 182 кода `ErrorsCodeList` (RU/UA/EN, `DT/ErrorsCodeList.cs`), из них возбуждаются
  142; 37 мёртвых + 3 только в закомментированном коде (§6). Все — ошибки; предупреждений нет.
